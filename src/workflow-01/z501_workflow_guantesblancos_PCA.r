...
Copiar_Dataset <- function(dataset_dir)
{
  # Extract file name
  file_name <- basename(dataset_dir)
  
  # Create new file name with "_original" appended
  new_file_name <- paste0(tools::file_path_sans_ext(file_name), "_original.", tools::file_ext(file_name))
  
  # Construct full path for original and new file
  original_path <- dataset_dir
  new_path <- file.path(dirname(dataset_dir), new_file_name)
  
  # Copy original dataset to new file
  return(file.copy(original_path, new_path))
}

Reduccion_Dimensionalidad <- function(dataset_dir){
  
  # Llamar a la función para copiar el dataset
  if (!Copiar_Dataset(dataset_dir)) {
  ...
  write.csv(dataset_componentes, dataset_dir, row.names = FALSE)
